...
/**
 * servlet that adds a new channel to a registered user.
 * if the session is invalid or a registered user is not logged
 * in, he is redirected to the login page.
 * @author gokul
 */
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response){
		try{
			...
			if(session != null && session.getAttribute("username") != null){
				...
				writer.print("XSLT link <input type=\"text\" name=\"xslturl\"><br>");
				writer.print("Xpaths <textarea name=\"xpaths\" rows=\"10\" cols=\"50\"></textarea><br>");
		...
		try{
			...
			if(session == null || session.getAttribute("username") == null){
				...
				if(c!= null){
					...
					cs.createChannel(username, channelName, xpath, xslturl);
